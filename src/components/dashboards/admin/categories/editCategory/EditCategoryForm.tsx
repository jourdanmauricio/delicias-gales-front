...
const EditCategoryForm = ({ category }) => {
  const { data, selectedFile, preview, loading, errors, onSelectFile, handleChange, handleCancel, handleSubmit } = useEditCategoryForm({ category });
  ...
  return (
    ...
    >
      ...
      <div className='col-span-2 flex justify-between'>
        ...
        <button type='submit' className='btn btn-confirm'>{data.id === 'new' ? 'Crear' : 'Modificar'}</button>
      </div>
    </form>
  )
}
