...
const NewEditUser = ({ loading, errors, currentData, handleChange, action, hadleSubmit, handleCancel }) => {

  console.log(currentData)
  return (<>
    <div className='text-sm'><button onClick={handleCancel}>Usuarios</button> / {action === 'NEW' ? 'Nuevo' : 'Modificar'}</div>
    <form id='user-form' onSubmit={hadleSubmit} noValidate className='mt-8'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-4 items-center'>
        ...
        <div className="relative">
          ...
          <div className="mt-2">
            ...
            <p className={`input-error ${errors["role"] ? 'opacity-100' : 'opacity-0'}`}>
              {errors["role"]}
            </p>
          </div>
        </div>
      ...
      <div className='col-span-2 flex justify-between'>
        ...
        <button type='submit' className='btn btn-confirm'>{action === 'NEW' ? 'Crear' : 'Modificar'}</button>
      </div>
    </form>
  </>
  )
